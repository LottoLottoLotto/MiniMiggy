...
class FlashAttention:
    ...
    def __call__(self, q: Tensor, k: Tensor, v: Tensor, mask: Optional[Tensor] = None) -> Tensor:
        B, H, T, D = q.shape  # TinyGrad shape format
        out = Tensor.zeros((B, H, T, D))
        normalizers = Tensor.zeros((B, H, T, 1))
        m = Tensor.full((B, H, T, 1), float('-inf'))

        # ✅ Compute mask dynamically per batch
        if mask is None:
            mask = Tensor.ones((T, T)).tril().cast(dtypes.bool).reshape(1, 1, T, T)
        ...
        for block_start in range(0, T, self.block_size):
            ...
            # ✅ Compute attention scores safely
            scores = (q @ k_block.transpose(-2, -1)) * self.softmax_scale

            # ✅ Apply mask safely using `.masked_fill()` (instead of `where()`)
